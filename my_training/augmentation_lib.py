...
def random_reverse(images: Dict[str, tf.Tensor]) -> Dict[str, tf.Tensor]:
    """Reverses frames from 'frame0' to 'frame9' randomly without specific keys.

    Args:
      images: A dictionary containing frame keys from 'frame0' to 'frame9'.

    Returns:
      A dictionary with frames reversed if the condition is met.
    """
    prob = tf.random.uniform((), minval=0, maxval=2, dtype=tf.int32)
    prob = tf.cast(prob, tf.bool)
    ...
    def _reverse(images):
        # Reverse order of frames from frame0 to frame9
        reversed_images = {}
        for i in range(6):
            reversed_images[f'frame{i}'] = images[f'frame{5 - i}']
