...
import losses.vgg19_loss as vgg19
...
def l1_loss(example: Mapping[str, tf.Tensor],
  ...
  y = example['y']
  if y.shape[-1] == 1:  # If the image is grayscale, convert it to RGB
    y = tf.image.grayscale_to_rgb(y)
  return tf.reduce_mean(tf.abs(prediction['image'] - y))
...
def l1_warped_loss(example: Mapping[str, tf.Tensor],
  ...
  if 'x0_warped' in prediction:
    loss += tf.reduce_mean(tf.abs(prediction['x0_warped'] - y))
  if 'x1_warped' in prediction:
    loss += tf.reduce_mean(tf.abs(prediction['x1_warped'] - y))
  return loss
...
def l2_loss(example: Mapping[str, tf.Tensor],
  ...
  if y.shape[-1] == 1:  # If the image is grayscale, convert it to RGB
    y = tf.image.grayscale_to_rgb(y)
  return tf.reduce_mean(tf.square(prediction['image'] - y))
...
def psnr_loss(example: Mapping[str, tf.Tensor],
  ...
  return tf.reduce_mean(
      tf.image.psnr(prediction['image'], y, max_val=1.0))
