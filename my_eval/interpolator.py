...
def save_flow_map(flow, filename):
...
def image_to_patches(image: np.ndarray, block_shape: List[int]) -> np.ndarray:
    ...
    """
    print("image_to_patches initial image shape:", image.shape)  # Original image shape
...
class Interpolator:
    ...
    def interpolate(self, x0: np.ndarray, x1: np.ndarray,
        ...
        results = self._model({'x0': x0, 'x1':x1}, training=False)
        frames = []
        for i in range(dt):

            frame = results[f'image_t{i}'][0].numpy()
            frames.append(frame)

        return frames


    def __call__(self, x0, x1, dt_values):
        return self.interpolate(x0, x1, dt_values)
